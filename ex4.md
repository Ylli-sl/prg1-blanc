### **Q4** / 5 (10 pts) - classe Image

Une image couleur est composée d'une grille rectangulaire de couleurs codées avec trois composantes RGB (Red, Green, Blue) auxquels on accède via leurs coordonnées (x,y) dans le plan. Ecrivez les structures et classe suivantes
...
Seule la méthode `subImage` doit être définie séparément.
...
Image Image::subImage(Coord p1, size_t w, size_t h) const {
   // validate input
   idx(p1);
   // Vérifie si le coin opposé est aussi dans l'image
