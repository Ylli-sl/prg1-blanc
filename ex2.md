### **Q2** / 5 (10 pts) - Consolidation de signaux WiFi

Nous avons réalisé des mesures de puissance de signaux de routeurs WiFi à intervalles réguliers sur une grande surface rectangulaire pour produire une carte de densité de puissance à 2 dimensions.
...
La carte des mesures est stockée dans un type `Reseau` à définir par vos soins comme un alias de `std::vector` de `std::vector` de `Routeur`.
